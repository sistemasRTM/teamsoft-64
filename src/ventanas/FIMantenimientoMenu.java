...
public class FIMantenimientoMenu extends MaestroInternalFrame {
	...
	public FIMantenimientoMenu() {
		setSize(507, 400);
		setTitle(getTitle() + "-" + Sesion.tifMenu);
		//
	...
	public void cargarTabla(List<Menu> menus) {
		limpiarTabla();
		for (Menu menu : menus) {
			Object datos[] = { menu.getCodigo(),
					menu.getDescripcion() };
			dtm.addRow(datos);
		}
	}

	public void cargarTabla(Menu menu) {
		limpiarTabla();
		Object datos[] = { menu.getCodigo(), menu.getDescripcion() };
		dtm.addRow(datos);
	}
